(function () {
  ...
  // ---- Furniture ---------------------------------------------------------
  // A plan reads as a plan because of what is drawn inside the rooms. Each
  // fixture has a natural size in grid cells and a viewBox of ten units per
  // cell, so the furniture is drawn to the same scale as the building and
  ...
  const FIXTURES = {
    ...
    waiting: { w: 3, h: 2, faces: "s", art:
      ...
      '<rect x="18.5" y="3" width="4" height="4" rx="0.5"/>' +
      '<rect x="24" y="3" width="4" height="4" rx="0.5"/>' +
      '<rect x="2" y="13" width="4" height="4" rx="0.5"/>' +
    ...
    shelving: { w: 2, h: 2, faces: "s", art:
      ...
      '<line x1="7" y1="15.8" x2="7" y2="19"/><line x1="13" y1="15.8" x2="13" y2="19"/>' },
  };
  const SYMBOL_FIXTURES = {
    bed: "bed", isolation: "bed", monitor: "monitor", ward: "ward", baby: "baby",
    emergency: "couch", treatment: "couch", consult: "couch", therapy: "couch",
    theatre: "theatre", imaging: "imaging",
    office: "office", staff: "office",
    reception: "reception", waiting: "waiting",
    toilet: "toilet", accessible: "toilet",
    lab: "counter", pharmacy: "counter", clean: "counter", utility: "counter",
    catering: "counter",
    store: "shelving", linen: "shelving", waste: "shelving", secure: "shelving",
    server: "shelving",
  };
  ...
  function renderFloorGrid(host, blocks, opts) {
    ...
    const area = (bounds.x0 + 1) + " / span " + Math.max(1, bounds.x1 - bounds.x0);
    const rowArea = (bounds.y0 + 1) + " / span " + Math.max(1, bounds.y1 - bounds.y0);
    const slab = document.createElement("div");
    slab.className = "fgslab";
    slab.setAttribute("aria-hidden", "true");
    slab.style.gridColumn = area;
    slab.style.gridRow = rowArea;
    host.appendChild(slab);
    const shell = document.createElement("div");
    shell.className = "fgshell";
    shell.setAttribute("aria-hidden", "true");
    shell.style.gridColumn = area;
    shell.style.gridRow = rowArea;
  ...
  function buildBlockEl(b, o, doorSide) {
    ...
    // A room only gets furniture if there is room to draw it legibly, and a
    // furnished room does not also get hatching — the fixtures say what it is.
    const fix = b.kind === "room" && b.active && w >= 2 && h >= 2
      ? fixtureFor(sym)
      : null;
    if (sym === "stairs") classes.push("fgblock--stairs");
    else if (sym === "lift") classes.push("fgblock--lift");
    else if (!fix && sym && HATCH_SYMBOLS.indexOf(sym) !== -1) classes.push("fgblock--hatch");

    let label = b.name;
    // A drawing labels a room with its name and its number. Everything else
    // (department, room type) stays in the accessible label and the inspector.
    const parts = [];
    if (b.code) parts.push(b.code);
    else if (b.room_type_name) parts.push(b.room_type_name);
